 ...
 * @access private
 */
class WC2_Test_List_Table extends WP_List_Table {
	...
	function __construct( $args = array() ) {
		...
		if ( !current_user_can( $post_type_object->cap->edit_others_posts ) ) {
			...
			$this->user_posts_count = $wpdb->get_var( $wpdb->prepare( "
				...
				AND post_author = %d AND post_mime_type = %s 
			", $post_type, get_current_user_id(), 'item' ) );
		...
		if ( 'post' == $post_type && $sticky_posts = get_option( 'sticky_posts' ) ) {
			...
			$this->sticky_posts_count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT( 1 ) FROM $wpdb->posts WHERE post_type = %s AND post_status NOT IN ('trash', 'auto-draft') AND ID IN ($sticky_posts) AND post_mime_type = %s", $post_type, 'item' ) );
		}
	}
