...
if( isset($this->payment_results['X-TRANID']) ){ //remise_card
	...
	$html .= '<table>'."\n";
	$html .= '<tr><th>'.__('オーダー番号', 'usces').'</th><td>' . $this->payment_results['order_no'] . "</td></tr>\n";
	switch($this->payment_results['pay_cvs']){
		case 'D001':
			$html .= '<tr><th>'.__('払込表番号', 'usces').'</th><td>' . $this->payment_results['pay_no1'] . "</td></tr>\n";
			$html .= '<tr><th>' . __('URL','usces') . '</th><td><a href="'.$this->payment_results['pay_url'].'" target="_blank">'.$this->payment_results['pay_url'] . '"</a></td></tr>'."\n";
			break;
		case 'D002':
			$html .= '<tr><th>'.__('お支払受付番号', 'usces').'</th><td>' . $this->payment_results['pay_no1'] . "</td></tr>\n";
			break;
		case 'D030':
			$html .= '<tr><th>'.__('注文番号', 'usces').'</th><td>' . $this->payment_results['pay_no1'] . "</td></tr>\n";
			$html .= '<tr><th>'.__('企業コード', 'usces').'</th><td>' . $this->payment_results['pay_no2'] . "</td></tr>\n";
			break;
		case 'D040':
			$html .= '<tr><th>'.__('ｵﾝﾗｲﾝ決済番号', 'usces').'</th><td>' . $this->payment_results['pay_no1'] . "</td></tr>\n";
			break;
		case 'D015':
			$html .= '<tr><th>'.__('お支払受付番号', 'usces').'</th><td>' . $this->payment_results['pay_no1'] . "</td></tr>\n";
			break;
	}
	$html .= '<tr><th>'.__('お支払期限', 'usces').'</th><td>' . substr($this->payment_results['pay_limit'], 0, 4).'年' . substr($this->payment_results['pay_limit'], 4, 2).'月' . substr($this->payment_results['pay_limit'], 6, 2).'日' . "(期限を過ぎますとお支払ができません)</td></tr>\n";
	$html .= '<!-- <tr><th>'.__('エラーコード', 'usces').'</th><td>' . $this->payment_results['error_code'] . "</td></tr> -->\n";
	$html .= '</table>'."\n";
	$html .= '<p>「お支払いのご案内」は、' . $entry['customer']['mailaddress1'] . '　宛にメールさせて頂いております。</p>'."\n";
	$html .= "</div>\n";
