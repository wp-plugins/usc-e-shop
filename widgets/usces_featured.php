...
class Welcart_featured extends WP_Widget {
    ...
    function widget($args, $instance) {	
		global $usces;
        extract( $args );
        $title = WCUtils::is_blank($instance['title']) ? 'Welcart '.__('Items recommended', 'usces') : $instance['title'];
        $icon = WCUtils::is_blank($instance['icon']) ? 1 : (int)$instance['icon'];
        $num = WCUtils::is_blank($instance['num']) ? 1 : (int)$instance['num'];
...
//			$myposts = get_posts('numberposts=' . $num . '&offset='.$offset.'&category='.usces_get_cat_id( 'itemreco' ));
    ...
    function form($instance) {				
        $title = WCUtils::is_blank($instance['title']) ? 'Welcart '.__('Items recommended', 'usces') : esc_attr($instance['title']);
		$icon = WCUtils::is_blank($instance['icon']) ? 1 : (int)$instance['icon'];
        $num = WCUtils::is_blank($instance['num']) ? 1 : (int)$instance['num'];
        ?>
