...
class Welcart_page extends WP_Widget {
    ...
    function widget($args, $instance) {		
        extract( $args );
        $title = WCUtils::is_blank($instance['title']) ? 'Welcart '.__('Page', 'usces') : $instance['title'];
    ...
    function form($instance) {				
        $title = WCUtils::is_blank($instance['title']) ? 'Welcart '.__('Page', 'usces') : esc_attr($instance['title']);
		$icon = WCUtils::is_blank($instance['icon']) ? 1 : (int)$instance['icon'];
        ?>
