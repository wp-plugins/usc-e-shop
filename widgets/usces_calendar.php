...
class Welcart_calendar extends WP_Widget {
    ...
    function widget($args, $instance) {		
        extract( $args );
        $title = WCUtils::is_blank($instance['title']) ? 'Welcart '.__('Business Calendar', 'usces') : $instance['title'];
    ...
    function form($instance) {				
        $title = WCUtils::is_blank($instance['title']) ? 'Welcart '.__('Business Calendar', 'usces') : esc_attr($instance['title']);
		$icon = WCUtils::is_blank($instance['icon']) ? 1 : (int)$instance['icon'];
        ?>
