...
class Welcart_category extends WP_Widget {
    ...
    function widget($args, $instance) {		
        extract( $args );
        $title = WCUtils::is_blank($instance['title']) ? 'Welcart '.__('Categories', 'usces') : $instance['title'];
        $cat_slug = WCUtils::is_blank($instance['cat_slug']) ? 'itemgenre' : $instance['cat_slug'];
    ...
    function form($instance) {				
        $title = WCUtils::is_blank($instance['title']) ? 'Welcart '.__('Categories', 'usces') : esc_attr($instance['title']);
        $cat_slug = WCUtils::is_blank($instance['cat_slug']) ? 'itemgenre' : esc_attr($instance['cat_slug']);
		$icon = WCUtils::is_blank($instance['icon']) ? 1 : (int)$instance['icon'];
        ?>
